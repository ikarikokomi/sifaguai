...
int main() {
	...
	while (true) {
		...
		if (commandtype == REGISTER) {
			...
			query_ticket(loc1, loc2, date, catalog, find_train, thetrain, theRestTicket);
		}
			...
			mystring<10> date = readDATE(it);
			mystring<10> catalog = readCATALOG(it);
			query_transfer(loc1, loc2, date, catalog, find_train, thetrain, location, theRestTicket);
		}
			...
			file_buy_ticket(id, intid, num, train_id, loc1, loc2, date, intdate, ticket_kind, thetrain, theticket, find_ticket, theRestTicket);
		}
			...
			mystring<20> ticket_kind = readTICKET_KIND(it);
			int intdate = readdate(date);
			file_refund_ticket(num, id, intid, train_id, loc1, loc2, date, intdate, ticket_kind, theticket, thetrain, theRestTicket);
		}

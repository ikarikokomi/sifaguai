...
public:
	...
	struct meta_t {
		...
		char ap[4096 - (3 *sizeof(off_t) + 6 * sizeof(size_t)) % 4096];
	};
	...
	struct internal_node_t {
		...
		off_t prev;
		size_t num;
		char ap[4096 - (3 *sizeof(off_t) + sizeof(size_t) + TREE_ORDER * sizeof(index_t)) % 4096];
	...
	struct leaf_node_t {
		...
		off_t prev;
		size_t num;
		char ap[4096 - (3 * sizeof(off_t) + sizeof(size_t) + TREE_ORDER * sizeof(record_t)) % 4096];
